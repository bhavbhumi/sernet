...
const About = () => {
  ...
  const [activeTab, setActiveTab] = useState<AboutTab>(
    tabParam && aboutTabs.includes(tabParam) ? tabParam : 'Company'
  );

  useEffect(() => {
    if (tabParam && aboutTabs.includes(tabParam) && tabParam !== activeTab) {
  ...
  return (
    <Layout>
      <SEOHead
        title="About SERNET"
        description="Learn about SERNET Financial Services — 35+ years of engaging, enabling and empowering prosperity globally across 54 cities and 18 countries."
        path="/about"
      />
      <PageHero
