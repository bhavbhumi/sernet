...
export default function AdminTickets() {
  ...
  const fetchAll = async () => {
    ...
    const [{ data: t }, { data: c }] = await Promise.all([
      db('support_tickets').select('*, crm_contacts(full_name, email, phone)').order('created_at', { ascending: false }),
  ...
  const handleCreate = async () => {
    ...
    if (form.contact_id) {
      ...
      if (c) {
        payload.contact_name = c.full_name;
        payload.contact_email = c.email;
        payload.contact_phone = c.phone;
      }
    } else {
    ...
    if (error) { toast({ title: 'Error', description: error.message, variant: 'destructive' }); }
      ...
      logAudit({ action: 'create', entity_type: 'support_tickets', details: { subject: form.subject } });
      toast({ title: 'Ticket created' });
      setDialogOpen(false);
      setForm({ subject: '', description: '', priority: 'medium', channel: 'phone', contact_id: '', contact_name: '', contact_email: '', contact_phone: '' });
      fetchAll();
    }
    setSaving(false);
  };
  ...
  const filtered = tickets.filter(t => {
    const match = (t.subject + ' ' + t.ticket_number + ' ' + (t.contact_name ?? '')).toLowerCase().includes(search.toLowerCase());
    const matchStatus = filterStatus === 'all' || t.status === filterStatus;
    const matchPri = filterPriority === 'all' || t.priority === filterPriority;
    return match && matchStatus && matchPri;
  });
  ...
  return (
    <AdminLayout
      ...
      subtitle="Manage support tickets and customer requests"
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-border">
              {loading ? (
                ...
                <tr key={t.id} className="hover:bg-muted/20 cursor-pointer" onClick={() => navigate(`/admin/support/tickets/${t.id}`)}>
                  ...
                  <td className="px-4 py-3 text-muted-foreground text-xs">{channelLabels[t.channel] ?? t.channel}</td>
