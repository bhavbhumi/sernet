...
// Maps DB identity keys to platform display names
const SOCIAL_PLATFORM_MAP: { key: string; name: string }[] = [
...
function buildSocialLinks(identity?: Record<string, any>) {
  if (!identity) {
    // Fallback defaults
    return [
      { name: 'Facebook', href: 'https://www.facebook.com/sernetfspl/' },
      { name: 'Instagram', href: 'https://www.instagram.com/sernetfspl' },
      { name: 'LinkedIn', href: 'https://www.linkedin.com/company/sernetfspl/' },
      { name: 'YouTube', href: 'https://www.youtube.com/@sernetfspl' },
      { name: 'WhatsApp', href: 'https://wa.me/919206767670' },
      { name: 'Telegram', href: 'https://t.me/sernetindia' },
    ];
  }
  return SOCIAL_PLATFORM_MAP
    .map(({ key, name }) => ({ name, href: (identity as any)[key] as string }))
      ...
      <div className="container-sernet py-8 md:py-10">
        ...
        <div className="mt-6 flex items-center justify-between gap-4 flex-wrap text-[13px]">
          ...
          <div className="inline-flex items-center gap-2.5">
            {buildSocialLinks(settings?.identity).map((social) => (
